...
object ApocalypseEventHandler {
    ...
    /**
     * Обрабатывает присоединение мобов к миру для увеличения количества агрессивных мобов
     */
    @SubscribeEvent
        ...
        if (entity is Monster && StorylineOfMineyarg.dataManager.mobEvolutionLevel > 0) {
            try {
                ...
                if (StorylineOfMineyarg.config.enableProgressLogging) {
                    StorylineOfMineyarg.LOGGER.debug("Применена эволюция к мобу: ${entity.javaClass.simpleName}")
                }
            } catch (e: Exception) {
                StorylineOfMineyarg.LOGGER.error("Ошибка при применении эволюции к мобу: ${e.message}")
            }
        }
    }
    
    /**
     * Спавнит дополнительных мобов рядом с исходным (для тестового режима)
     */
    private fun spawnAdditionalMobs(originalMob: Monster) {
        try {
            ...
            // Спавним 1-2 дополнительных моба
            ...
            repeat(additionalMobCount) {
                ...
                if (level.addFreshEntity(newMob)) {
                    if (StorylineOfMineyarg.config.enableProgressLogging) {
                        StorylineOfMineyarg.LOGGER.debug("Заспавнен дополнительный моб в тестовом режиме")
                    }
                }
            }
        } catch (e: Exception) {
            StorylineOfMineyarg.LOGGER.error("Ошибка при спавне дополнительных мобов: ${e.message}")
        }
    }
}